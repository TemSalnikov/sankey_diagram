    ...
    for filename in FILENAMES:
        ...
        for page in file:
            ...
            while i < len(rows) - 1:
                ...
                transfer_date,  trans_sum_str, debit, credit, oper_text = rows[i + idx_transfer_date:i + num_fields]
                next_col = i + num_fields


                debit = unidecode(debit).replace(' ', '').replace(',', '')
                credit = unidecode(credit).replace(' ', '').replace(',', '')
                while next_col < len(rows) - 2 and not (pat_date.search(rows[next_col]) and len(rows[next_col]) == 10):
                    oper_text = ' '.join((oper_text, rows[next_col]))
                    next_col += 1

                oper_text = re.split('RUB', oper_text)[1]
